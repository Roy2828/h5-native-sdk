    ...
    public void deniedPermissionRequest(@NonNull Activity activity, @NonNull List<String> allPermissions,
        ...
        if (doNotAskAgain) {
            if (deniedPermissions.size() == 1 && Permission.ACCESS_MEDIA_LOCATION.equals(deniedPermissions.get(0))) {
                ToastUtil.normal(activity.getString(R.string.common_permission_media_location_hint_fail));
                return;
            }
        ...
        if (deniedPermissions.size() == 1) {
            ...
            if (Build.VERSION.SDK_INT >= 30) {
                backgroundPermissionOptionLabel = "后台定位权限";
            }
            ...
            if (Permission.ACCESS_BACKGROUND_LOCATION.equals(deniedPermission)) {
                ToastUtil.normal(activity.getString(R.string.common_permission_background_location_fail_hint, backgroundPermissionOptionLabel));
                return;
            }

            if (Permission.BODY_SENSORS_BACKGROUND.equals(deniedPermission)) {
                ToastUtil.normal(activity.getString(R.string.common_permission_background_sensors_fail_hint, backgroundPermissionOptionLabel));
                return;
            }
        }
